...
export default abstract class AbstractAISDKModel implements ModelInterface {
  ...
  public async chat(messages: Message[], options: CallChatCompletionOptions): Promise<StreamTextResult> {
    try {
      console.log("Model printing === message == krishna"+ JSON.stringify(options))
      console.log("Model printing === message == krishna"+ JSON.stringify(messages))
  ...
  private async _callChatCompletion<T extends ToolSet>(
    ...
    console.log("Model printing === message == krishna"+ JSON.stringify(model))
    ...
    console.info('stream chunk full', JSON.stringify(result))
    let contentParts: MessageContentParts = []
    let currentTextPart: MessageTextPart | undefined = undefined
    let reasoningContent = ''
    console.info('stream chunk after 1 one time', JSON.stringify(result.fullStream))
    for await (const chunk of result.fullStream) {
      console.info('stream chunk after 1', JSON.stringify(chunk))
    ...
    console.log("contentParts ============= "+ JSON.stringify(contentParts))
    console.log("contentParts ============= reasoningContent "+ JSON.stringify(contentParts))
    console.log("contentParts ============= usage "+ JSON.stringify(contentParts))
